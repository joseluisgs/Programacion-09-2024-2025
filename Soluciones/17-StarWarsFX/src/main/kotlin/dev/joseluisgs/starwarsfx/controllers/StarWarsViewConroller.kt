...
class StarWarsViewConroller : KoinComponent {
    ...
    private fun initBindings() {
        ...
        viewModel.state.addListener { _, oldValue, newValue ->
            ...
            Platform.runLater {
                ...
                if (oldValue.resultado != newValue.resultado) {
                    // Si el resultado ha cambiado, actualizamos el estado
                    logger.debug { "Resultado cambiado" }
                    newValue.resultado.onSuccess {
                        when (it) {
                            is StarWarsResultado.StarWarsFin -> onTerminadoAction()
                            is StarWarsResultado.StarWarsEjecutandose -> onLoadingAction()
                            StarWarsResultado.StarWarsInicio -> return@onSuccess
                        }
                    }
                }
    ...
    private fun initEvents() {
        logger.debug {
            ...
            menuSalir.setOnAction { onCloseAction() }
