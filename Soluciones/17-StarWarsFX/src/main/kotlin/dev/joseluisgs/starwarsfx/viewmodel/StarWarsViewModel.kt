...
import dev.joseluisgs.starwarsfx.errors.StarWarsError
...
class StarWarsViewModel(
    ...
    init {
        logger.debug { "StarWarsViewModel cargando datos" }
    }

    fun comenzarSimulacion(dimensionEspacio: Int, tiempo: Int, droidesTotal: Int) {
        logger.debug { "Comenzando simulación" }

        // Actualizamos el estado con los datos de la simulación inicial
        state.value = state.value.copy(
            dimensionEspacio = dimensionEspacio,
            tiempoTotal = tiempo,
            droidesTotal = droidesTotal,
            //resultado = Ok(StarWarsResultado.StarWarsEjecutandose)
        )

        actualizarEstadoSimulacion(StarWarsResultado.StarWarsEjecutandose)
    ...
    private fun initDroidesEnemigos() {
        ...
        repeat(state.value.droidesTotal) {
    ...
    fun simulacion() {
        ...
        // Actualizo el estado de la simulacion
    }

    private fun actualizarEstadoSimulacion(starWarsEjecutandose: StarWarsResultado) {
        state.value = state.value.copy(resultado = Ok(starWarsEjecutandose))
        println("Estado de la simulacion: $starWarsEjecutandose")
    ...
    private fun estadoResultadoFinal() {
        actualizarEstadoSimulacion(StarWarsResultado.StarWarsFin("Simulación terminada"))
    }
    ...
    data class StarWarsState(
        ...
        val aciertos: Int = 0,
        val muertos: Int = 0,
        val resultado: Result<StarWarsResultado, StarWarsError> = Ok(StarWarsResultado.StarWarsInicio)
    )
