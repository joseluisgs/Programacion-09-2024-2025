...
class HelloController {
    ...
    private fun initialize() {
        ...
        textViewModel.textProperty().bindBidirectional(DemoViewModel.mensaje) // es bidireccional, porque si cambia el texto en el modelo cambia la vista y viceversa
        numVeces.textProperty().bind(DemoViewModel.numVeces.asString()) // no es bidireccional, porque no cambia
        //textViewModel.textProperty().bind(DemoViewModel.mensaje) // es unidireccional, porque si cambia el texto en la vista no cambia el modelo
        
       /* textViewModel.textProperty().addListener { _, oldValue, newValue ->
            println("Nuevo valor: $newValue")
            if (oldValue != newValue) {
                println("El mensaje ha cambiado de $oldValue a $newValue")
                DemoViewModel.mensaje.value = newValue
            }
        }*/
        
        
        
        DemoViewModel.mensaje.addListener { _, oldValue, newValue ->
            println("Nuevo valor: $newValue")
            /*if (oldValue != newValue) {
                println("El mensaje ha cambiado de $oldValue a $newValue")
                textViewModel.text = newValue
            }*/
        }
